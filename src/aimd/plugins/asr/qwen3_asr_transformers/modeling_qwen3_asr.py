...
def _compute_default_rope_parameters(config, device=None):  # noqa: ANN001
    """Return default RoPE parameters for Transformers versions without a default entry."""
...
class Qwen3ASRThinkerTextRotaryEmbedding(nn.Module):
    ...
    def compute_default_rope_parameters(self, config=None, device=None):  # noqa: ANN001
        """Compatibility hook expected by Transformers 5 weight initialization."""
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if cache_position is None:
            ...
            prepared_input_ids = model_inputs.get("input_ids", input_ids)
            sequence_length = prepared_input_ids.shape[-1]
            cache_position = (
                torch.arange(
                    sequence_length,
                    device=prepared_input_ids.device,
                )
                + past_seen_tokens
            )
